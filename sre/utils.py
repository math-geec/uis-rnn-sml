...
def random_batch(speaker_num=config.N, utter_num=config.M, shuffle=True, noise_filenum=None, utter_start=0,
    ...
    # TI-SV
        ...
        if shuffle:
            # np_file_list = [f for f in np_file_list if np.load(os.path.join(path,f)).shape[0]>=config.M]
            selected_files = random.sample(np_file_list, speaker_num)  # select random N speakers
        else:
            np_file_list = [f for f in np_file_list if np.load(os.path.join(path, f)).shape[0] >= 2 * config.M]
            selected_files = np_file_list[spk_start:spk_start + speaker_num]  # select first N speakers
        utter_batch = []
