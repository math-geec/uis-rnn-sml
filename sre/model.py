...
def test(path):
    ...
    # draw graph
    enroll = tf.placeholder(shape=[None, config.N * config.M, 20],
                            dtype=tf.float32)  # enrollment batch (time x batch x n_mel)
    verif = tf.placeholder(shape=[None, config.N * config.M, 20],
    ...
    with tf.Session() as sess:
        ...
        # Ensure enough utterance for each speaker
        test_spks = len([f for f in os.listdir(config.test_path) if
                         np.load(os.path.join(config.test_path, f)).shape[0] >= 2 * config.M])
        test_round = test_spks // config.N
        print("Total round of test : ", test_round)
        ...
        for i in range(test_round):
            ...
            S = sess.run(similarity_matrix, feed_dict={enroll: random_batch(shuffle=False, spk_start=i * config.N),
                                                       verif: random_batch(shuffle=False, spk_start=i * config.N,
        ...
        # Identification accuracy = correctly identified/total number of test data
        correct_identification = 0
        for i in range(test_round):
            for j in range(config.N):
                predicted_id = np.argmax(S_list[i][j], axis=0)
                correct_identification += np.sum(predicted_id == j)
        ACC = correct_identification / config.N / config.M / test_round
        ...
        for thres in [0.01 * i for i in range(100)]:
            S_thres = [S > thres for S in S_list]

            # False acceptance ratio = false acceptance / mismatched population (enroll speaker != verification speaker)
            FAR = sum([sum([np.sum(S_thres[j][i]) - np.sum(S_thres[j][i, :, i]) for i in range(config.N)]) for j in
                       range(test_round)]) / (config.N - 1) / config.M / config.N / test_round

            # False reject ratio = false reject / matched population (enroll speaker = verification speaker)
            FRR = sum([sum([config.M - np.sum(S_thres[j][i][:, i]) for i in range(config.N)]) for j in
                       range(test_round)]) / config.M / config.N / test_round
